...
def get_historical_conversion_rate(
    row,
    target_currency: str,
...
def customize_page_appearance() -> None:
    st.set_page_config(page_title="Rich Secret", page_icon="💰")
...
def settings(budget_sheets, now_gmt4):
    col1, col2, _col4, col3 = st.columns([1, 1, 0.5, 0.5])
    st.session_state.mode_not_selected = False
    mode = col1.pills(
        "Mode",
    ...
    col2.caption(
        f"<p style='text-align: center;'>Date range: {start_date.strftime('%d %b, %Y')}"
        f" to {end_date.strftime('%d %b, %Y')}</p>",
        unsafe_allow_html=True,
    )
    ...
    if st.session_state.mode_not_selected:
        col2.caption(
            '<p style="text-align: center;">Showing "Month" mode.</p>',
            unsafe_allow_html=True,
        )
    if mode == "Custom":
        col2.caption(
...
def calculate_stats(budget_data, start_date, end_date):
    ...
    planned_income = monthly_plan_data[monthly_plan_data["type"] == "income"][
        "converted_amount"
    ].sum()
...
def stats(mode, start_date, end_date):
    ...
    precision = (
        ...
        if st.session_state.target_currency == "RUB"
        else 2
    )
    ...
    if mode != "Custom":
        ...
        deltas = {
            ...
            "actual_weekly_spend": this_period_stats["actual_weekly_spend"]
            - prev_period_stats["actual_weekly_spend"],
            "can_spend_weekly": this_period_stats["can_spend_weekly"]
            - prev_period_stats["can_spend_weekly"],
        }
    else:
        deltas = {
            "balance": None,
    ...
    for col, metric_name, this_period_value, delta in zip(
        ...
        col.metric(
            ...
            help=(
                str(round(this_period_value, 10))
                if st.session_state.target_currency == "BTC"
                else str(round(this_period_value, 2))
            ),
        )

    actual_weekly_spend_full = (
        round(this_period_stats["actual_weekly_spend"], 10)
        if st.session_state.target_currency == "BTC"
        else str(round(this_period_stats["actual_weekly_spend"], 2))
    )
    expected_weekly_spend_full = (
        round(this_period_stats["can_spend_weekly"], 10)
        if st.session_state.target_currency == "BTC"
        else str(round(this_period_stats["can_spend_weekly"], 2))
    )

    this_period_stats["actual_weekly_spend"] = (
        0
        if np.isnan(this_period_stats["actual_weekly_spend"])
        else this_period_stats["actual_weekly_spend"]
    )
    deltas["actual_weekly_spend"] = (
        None
        if deltas["actual_weekly_spend"] is None
        else this_period_stats["actual_weekly_spend"]
    ...
    if deltas["actual_weekly_spend"] is not None:
        percentage = (
            None
            if prev_period_stats["actual_weekly_spend"] == 0
            or np.isnan(prev_period_stats["actual_weekly_spend"])
            else round(
                deltas["actual_weekly_spend"]
                / np.abs(prev_period_stats["actual_weekly_spend"])
                * 100,
            )
        )
...
def create_plot_data_actual_vs_planned(
    ...
    actual_sums = (
        actual_data.groupby("category", observed=True)["converted_amount"]
        .sum()
...
def add_actual_vs_planned_subplot(
    fig,
    plot_data,
    row,
    col,
...
def actual_vs_planned_plot(hide_fixed):
    st.markdown(
        "<h5><span style='color:lightseagreen'>Actual</span>"
    ...
    fig.update_xaxes(
        title=st.session_state.target_currency,
        showline=False,
        showgrid=True,
...
def add_balance_lineplot(aggregate_by, start_date, end_date, fig, row, col):
    ...
    all_data = (
        all_data.groupby([pd.Grouper(key="date", freq=freq), "type"])
        .agg({"converted_amount": "sum"})
        .reset_index()
    )
    ...
    date_range = pd.date_range(
        start=all_data["date"].min() - pd.to_timedelta(freq),
    ...
    balance_data.loc[0, "balance"] = (
        balance_data.loc[1, "balance"] - balance_data.loc[1, "converted_amount"]
    )
    ...
    for data_type, color in zip(
        ["Expense", "Income", "Savings"][::-1],
...
def add_savings_stacked_area(aggregate_by, start_date, end_date, fig, row, col):
    ...
    numeric_cols = ["amount_x", "amount_y", "converted_amount_x", "converted_amount_y"]
    ...
    zeros_df = pd.DataFrame(
        [
            {
                "date": start_date - pd.to_timedelta("1D"),
                "category": category,
                "amount": 0,
                "converted_amount": 0,
            }
    ...
    savings["converted_amount"] = savings.groupby("category", observed=True)[
        "converted_amount"
    ].cumsum()
    savings["amount"] = pd.to_numeric(savings["amount"], errors="coerce")
    savings["amount"] = savings.groupby("category", observed=True)["amount"].cumsum()
    full_date_range = pd.date_range(start=start_date, end=end_date, freq="1D")

    savings = (
        savings.drop(columns="name")
    ...
    true_amount = (
        savings.pivot(columns="category", values="amount")  # noqa: PD010
        .reindex(full_date_range)
        .ffill()
        .fillna(0)
    )
    ...
    currency = (
        savings.pivot(columns="category", values="currency")  # noqa: PD010
        .reindex(full_date_range)
        .ffill()
    )
    ...
    for c, column in enumerate(plot_df.columns):
        ...
        fig.add_trace(
            go.Scatter(
                ...
                line_shape="spline",
                hovertemplate=(
                    hovertemplate.replace("{{column}}", column).replace(
                        "{{currency}}",
                        st.session_state.target_currency,
                    )
                ),
...
def trends_plot(aggregate_by, hide_fixed, start_date, end_date):
    ...
    fig, data_start_date, data_end_date = add_balance_lineplot(
        aggregate_by,
        start_date,
        end_date,
        fig,
...
3. Input your values in the template.
    ...
    st.caption(
        "P.S. If you just want to see what the app looks "
...
def main():
    ...
    if st.session_state.google_doc_id:
        try:
            ...
            st.error(
                f"Google Sheets document probably has restricted access. Got an error: {e}\n\n"
