...
def create_histogram(data: pd.DataFrame,
    ...
    if np.issubdtype(data[data_col].dtype, np.datetime64):
        ...
        if not bin_width and n_bins:
            ...
            bins = pd.date_range(data[data_col].min() - bin_width,
                                 data[data_col].max() + 2*bin_width, freq=bin_width)
        elif bin_width:
        ...
        bins = np.linspace(data[data_col].min() - bin_width,
                           data[data_col].max() + 2*bin_width, n_bins + 1)
...
def create_kde_lines(data: pd.DataFrame,
    ...
    for category, group in data.groupby(category_col):
        ...
        if len(x) < 2:
            ...
            if is_datetime:
                x_min, x_max = pd.Timestamp(x_min, unit="s"), pd.Timestamp(x_max, unit="s")
...
def create_outline(x: np.array, y: np.array, x_diff: np.array, smoothing) -> tuple[np.array]:
...
def _check_input(ridgetype, show_points):
    ...
    if show_points not in ["all", "outliers", "none", True, False]:
        raise ValueError(f"Incorrect show_points value: {show_points}. "
                         "Should be one of 'all', 'outliers', 'none', True, False")


def plot_points(fig, show_points, spread, ridgetype,
                initial_data, data_col, stats_col_init, initial_x, y,
                array_dict, categories_list, category, index, category_col,
                jitter_max_height, jitter_strength_x, jitter_strength_y, points_size,
                lower_fence, upper_fence, edgecolor, color, hoverdata, row, col,
                ):
    if show_points != "none" and show_points is not False:
        jitter_strength_x_new = jitter_strength_x * spread / 100
        jitter_amount_x = (
            pd.to_timedelta(
                np.random.uniform(-jitter_strength_x_new,
                                  jitter_strength_x_new, len(initial_x)),
                unit="s")
            if np.issubdtype(initial_x.dtype, np.datetime64)
            else np.random.uniform(-jitter_strength_x_new,
                                   jitter_strength_x_new, len(initial_x))
        )
        ...
        if stats_col_init is None:
            ...
            jitter_y = np.repeat(len(categories_list)-index-1, len(initial_x)) + jitter_amount_y
        else:
            initial_y = (
                initial_data[initial_data[category_col] == category][stats_col_init]
                * array_dict[f"{category}_norm_factor"] * jitter_max_height
                + len(categories_list)-1-index
            ) if ridgetype == "bins" else (
                initial_data[initial_data[category_col] == category][stats_col_init]
                * array_dict[f"{category}_norm_factor"] * (
                    array_dict[f"y_{category}_normalized"].max() /
                    (initial_data[initial_data[category_col] == category][stats_col_init]
                     * array_dict[f"{category}_norm_factor"]).max()
                ) * jitter_max_height + len(categories_list)-1-index
            ) if array_dict[f"y_{category}_normalized"].max() != 0 else (
                initial_data[initial_data[category_col] ==
                             category][stats_col_init] * 0+jitter_max_height / 2
                + len(categories_list)-1-index
            )
            jitter_strength_y_new = jitter_strength_y * y.max() / 100
            jitter_amount_y = np.random.uniform(-jitter_strength_y_new,
                                                jitter_strength_y_new, len(initial_y))
        ...
        if not stats_col_init:
            ...
            customdata = initial_data[initial_data[category_col] == category][stats_col_init]
        else:
            hoverdata = [hoverdata] if isinstance(hoverdata, str) else hoverdata
            for col_ in hoverdata:
                if (pd.api.types.is_categorical_dtype(initial_data[col_])
                        and "NA" not in initial_data[col_].cat.categories):
                    initial_data[col_] = initial_data[col_].cat.add_categories("NA")
            customdata_rest = [initial_data[initial_data[category_col] == category][col].fillna("NA")
                               for col in hoverdata]
            customdata = np.stack(
                (initial_data[initial_data[category_col] == category][stats_col_init],
                 *customdata_rest), axis=-1)
            hovertemplate = (
                "{{Data}}: %{x}<br>"
                ...
                "{{col}}: %{customdata[{{c}}]}"
                .replace("{{col}}", str(col)).replace("{{c}}", str(c+1))
...
def extend_x_y(x, y, supp_x_long, threshold_fraction=0.01):
    ...
    if is_datetime:
        ...
        supp_x_long_numeric = (supp_x_long - pd.Timestamp("1970-01-01")) / pd.Timedelta(seconds=1)
    else:
        x_numeric = x
    ...
    pre_x, pre_y, post_x, post_y = np.array([]), np.array([]), np.array([]), np.array([])

    # Generate 100 points for each required segment (avoid adding if the diff is too small)
    if min_diff > threshold:  # Only generate pre-extension if the difference is large enough
        pre_x = np.linspace(supp_min, x_numeric.min(), 100)
        pre_y = np.zeros_like(pre_x)

    if max_diff > threshold:  # Only generate post-extension if the difference is large enough
...
def create_line_hovertemplate(data_col, stats_col_init,
                              initial_ridgetype, ridgetype,
                              hover_stats,
                              initial_x, upper_fence, lower_fence,
                              y_outline, smoothing, array_dict, category, supp_x_long):
    hovertemplate = (
        "{{Data}}: %{x}<br>"
        "{{Duration}}: %{customdata}"
        .replace("{{Data}}", data_col)
        .replace("{{Duration}}", stats_col_init if stats_col_init and initial_ridgetype == "bins"
                 else "count" if ridgetype == "bins" else "density"))

    hovertemplate = hovertemplate if not hover_stats else (
        hovertemplate+"<br><br>" +
        "max: {{max}}<br>"
    ...
    customdata = (
        y_outline if initial_ridgetype == "bins"
        and smoothing == 0 else extend_x_y(
            array_dict[f"x_{category}"], array_dict[f"y_{category}"], supp_x_long)[1])
...
def ridgeline(
    ...
    **Default Behavior**:
    ...
    if ridgetype == "bins":
        data = create_histogram(data, category_col, data_col, stats_col, bin_width, n_bins)
        stats_col = "count"
    ...
    array_dict = _create_array_dict(data, category_col, "Data",
                                    normalize_each, scaling_factor)
    categories_list = categories_order if categories_order is not None else sorted(
        data[category_col].unique())
    ...
    for index, category in enumerate(categories_list):
        ...
        supp_x_small = (
            ...
            if (ridgetype == "bins") and not smoothing else
        ...
        x, y = (x, y) if (ridgetype == "lines") or smoothing else create_outline(
            x, y, x_diff, smoothing)
        x, y = extend_x_y(x, y, supp_x_long)

        initial_x = initial_data[initial_data[category_col] == category][data_col]
        if bin_width == "1D":
            initial_x = initial_x + x_diff[-1] / 2 if initial_ridgetype == "bins" else initial_x
        upper_fence = (initial_x.quantile(0.75)
                       + 1.5 * (initial_x.quantile(0.75) - initial_x.quantile(0.25)))
        lower_fence = (initial_x.quantile(0.25)
                       - 1.5 * (initial_x.quantile(0.75) - initial_x.quantile(0.25)))

        hovertemplate, customdata = create_line_hovertemplate(
            data_col, stats_col_init,
            initial_ridgetype, ridgetype,
            hover_stats,
            initial_x, upper_fence, lower_fence,
            y_outline, smoothing, array_dict, category, supp_x_long)
        ...
        spread = (
            (initial_data[data_col].max() - initial_data[data_col].min()).total_seconds()
            if np.issubdtype(initial_x.dtype, np.datetime64)
            else (initial_data[data_col].max() - initial_data[data_col].min())
        )

        plot_points(fig, show_points, spread, initial_ridgetype,
                    initial_data, data_col, stats_col_init, initial_x, y,
                    array_dict, categories_list, category, index, category_col,
                    jitter_max_height, jitter_strength_x, jitter_strength_y, points_size,
                    lower_fence, upper_fence, edgecolor, color, hoverdata,
                    row=row, col=col)
